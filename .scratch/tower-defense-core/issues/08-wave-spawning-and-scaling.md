...
- Balance pass (2026-08-30): Wave scaling tightened. Count is now `5 + n + floor(0.1 * n * n)` (super-linear, so later Waves spike), per-Wave Health scaling raised from `1 + n * 0.1` to `1 + n * 0.15`, and the spawn interval now shrinks 0.8s − 0.02s per Wave, flooring at 0.3s. The `5 + n` / `0.1` / 0.8s figures above document the game as it then stood; the live values are the `WAVE_*` and `SPAWN_INTERVAL_*` constants.
